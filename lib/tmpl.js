...
function _funcParse(name, args){
    ...
    if(/if/.test(name)){ // if & else if
        ...
        args = args.replace(/\(([^\(]*)\)/g, function (w, m1) { // 编码括号里的逗号，以支持split
            return "(" + m1.replace(/,/g, "#comma#") + ")";
        }).split(",");
        for (var i = 0, len = args.length; i < len; ++i) {
            _args.push(_funcArgsParse(args[i].replace(/#comma#/g,","))); // 把编码后的逗号改回来
        }
...
function _$$Handler(str){
    if(/\(|\)|#br#/.test(str) || str.length < 1 || str.length > 50) return true;
    return false;
}
function tmpl(str, data, path, encoding) {
    ...
    str = str
        .replace(/\$(.*?)(?:.DATA)?\$/g, function(w, m1){ // <em>$xxx$</em> -> '<em>' + xxx + '</em>'
            if(!data[m1]) data[m1] = "";
    ...
    var loopReg = /<%(.*?)\v(.*?)<%\/\1\v/g;
    while(loopReg.test(str)){
        str = str
            .replace(loopReg, function(w, m1, m2){
    ...
    str = str
        .replace(/\f/g, "');")
        .replace(/\v/g, "p.push('");
    var fnStr = "var p=[]; with(obj){p.push('" + str + "');}return p.join('');";
